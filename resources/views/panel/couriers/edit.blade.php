...
<div class="mb-6">
    <a href="{{ route('panel.couriers.show', $courier) }}" class="text-gray-600 hover:text-gray-800 flex items-center">
...
<div class="bg-white rounded-xl shadow-sm p-6">
    <form method="POST" action="{{ route('panel.couriers.update', $courier) }}">
        ...
        <div class="mt-6 space-y-4">
            ...
            @php
                $currentRegions = $courier->courierRegions->pluck('id')->toArray();
            @endphp
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                        @foreach($regions as $region)
                            <label class="region-item flex items-center p-4 border rounded-lg cursor-pointer hover:bg-gray-50 transition-colors
                                          {{ in_array($region->id, old('region_ids', $currentRegions)) ? 'bg-green-50 border-green-300' : '' }}">
                                <input type="checkbox" name="region_ids[]" value="{{ $region->id }}"
                                       ...
                                       {{ in_array($region->id, old('region_ids', $currentRegions)) ? 'checked' : '' }}>
...
<div class="bg-white rounded-xl shadow-sm p-6 mt-6">
    ...
    <form method="POST" action="{{ route('panel.couriers.reset-password', $courier) }}">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="password" name="password_confirmation" required autocomplete="new-password"
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-black focus:border-black @error('password_confirmation') border-red-500 @enderror"
        ...
        <p class="mt-2 text-xs text-gray-500">Şifre en az 8 karakter olmalıdır.</p>
