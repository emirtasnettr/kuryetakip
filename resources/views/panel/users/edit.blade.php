...
<div class="max-w-3xl mx-auto">
    ...
    <form method="POST" action="{{ route('panel.users.update', $user) }}" class="space-y-6">
        ...
        <!-- Bölge Ataması -->
        @php
            $courierDistricts = $user->courierDistricts->pluck('id')->toArray();
            $authorizedDistricts = $user->authorizedDistricts->pluck('id')->toArray();
            $primaryDistrictId = $user->courierDistricts->where('pivot.is_primary', true)->first()?->id;
            $courierCity = $user->courierDistricts->first()?->city;
            $authorizedDistrictsByCity = $user->authorizedDistricts->groupBy('city');
        @endphp
        ...
        <div id="courierDistrictFields" class="bg-white rounded-xl shadow-sm p-6 hidden">
            ...
            <div class="mb-4">
                ...
                <select id="courierCitySelect" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-black focus:border-black">
                    ...
                    @foreach($cities as $city)
                        <option value="{{ $city }}" {{ $courierCity == $city ? 'selected' : '' }}>{{ $city }}</option>
                    @endforeach
                </select>
                    ...
                    @foreach($districts as $district)
                        <label class="courier-district-item flex items-center p-2 hover:bg-gray-50 rounded {{ $courierCity == $district->city ? '' : 'hidden' }}" data-city="{{ $district->city }}">
                            <input type="checkbox" name="districts[]" value="{{ $district->id }}"
                                   {{ in_array($district->id, $courierDistricts) ? 'checked' : '' }}
            ...
            <!-- Ana Bölge -->
            <div id="primaryDistrictField" class="{{ $courierCity ? '' : 'hidden' }}">
                ...
                <select id="primary_district" name="primary_district"
                    ...
                    @foreach($districts as $district)
                        <option value="{{ $district->id }}" data-city="{{ $district->city }}" 
                                class="courier-primary-option {{ $courierCity == $district->city ? '' : 'hidden' }}"
                                {{ $primaryDistrictId == $district->id ? 'selected' : '' }}>
                            {{ $district->name }}
                        </option>
                    @endforeach
                </select>
        ...
        <div id="staffDistrictFields" class="bg-white rounded-xl shadow-sm p-6 hidden">
            ...
            <div id="noCityMessage" class="text-center py-8 text-gray-500 {{ $authorizedDistrictsByCity->count() > 0 ? 'hidden' : '' }}">
...
// Mevcut yetkili bölgeler
const existingAuthorizedDistricts = @json($authorizedDistrictsByCity->map(fn($districts) => $districts->pluck('id')));
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Kurye için mevcut seçimleri yükle
    updateCourierPrimaryOptions();
    
    // Operasyon personeli için mevcut şehir gruplarını yükle
    Object.keys(existingAuthorizedDistricts).forEach(city => {
        const districtIds = existingAuthorizedDistricts[city];
        addCityGroup(city, districtIds);
    });
});
</script>
@endpush
