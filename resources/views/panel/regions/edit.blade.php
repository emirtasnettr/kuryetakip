    ...
    <div class="bg-white rounded-xl shadow-sm">
        ...
        <div class="p-6 border-b">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-xl font-semibold">Bölge Düzenle</h1>
                    <p class="text-sm text-gray-500">{{ $region->name }}</p>
                </div>
            </div>
        </div>

        <form action="{{ route('panel.regions.update', $region) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <div class="city-dropdown">
                        ...
                        <input type="hidden" name="city" id="city-value" value="{{ old('city', $region->city) }}">
                        <div class="city-dropdown-list" id="city-list">
                            @foreach($cities as $city)
                                <div class="city-dropdown-item {{ old('city', $region->city) == $city ? 'selected' : '' }}" 
                                     data-value="{{ $city }}" data-search="{{ $city }}">
                                    {{ $city }}
                                </div>
                            @endforeach
                        </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <div class="flex items-center gap-3">
                            <input type="color" name="color" id="color-picker" value="{{ old('color', $region->color) }}" 
                                   class="w-12 h-10 border border-gray-300 rounded-lg cursor-pointer">
                            <input type="text" id="color-text" value="{{ old('color', $region->color) }}" 
                        ...
                        <div class="flex gap-2 mt-2">
                            @foreach(['#3B82F6', '#10B981', '#F59E0B', '#EF4444', '#8B5CF6', '#EC4899', '#06B6D4', '#84CC16'] as $preset)
                                <button type="button" onclick="setColor('{{ $preset }}')" 
                                        class="w-6 h-6 rounded-full border-2 border-transparent hover:border-gray-400 transition-colors"
                    ...
                    <div>
                        ...
                        <select name="is_active" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-black focus:border-black">
                            <option value="1" {{ old('is_active', $region->is_active) ? 'selected' : '' }}>Aktif</option>
                            <option value="0" {{ !old('is_active', $region->is_active) ? 'selected' : '' }}>Pasif</option>
                        </select>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" 
                              ...
                              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-black focus:border-black">{{ old('description', $region->description) }}</textarea>
                </div>
            </div>
        ...
        <!-- Delete Form (hidden) -->
        <form id="delete-form" action="{{ route('panel.regions.destroy', $region) }}" method="POST" class="hidden">
            @csrf
            @method('DELETE')
        </form>
    </div>
